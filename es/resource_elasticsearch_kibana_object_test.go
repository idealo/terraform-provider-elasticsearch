	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testCheckElasticsearchKibanaObjectExists("elasticsearch_kibana_object.test_pattern", "index-pattern", "cloudwatch"),
				),
			},
			{
				Config:      testAccElasticsearchKibanaRejected,
...
func testCheckElasticsearchKibanaObjectExists(name string, objectType string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		case *elastic6.Client:
			_, err = client.Get().Index(".kibana").Type(objectType).Id(id).Do(context.TODO())
		default:
...
var testAccElasticsearchKibanaRejected = `
resource "elasticsearch_kibana_object" "test_rejected" {
  body = <<EOF
[
  {
		"_id": "foo:cloudwatch",
